...
>[!NOTE]
> * **Time complexity** depends on the **CPU speed** and the number of instructions the CPU can execute in a given time.
> * **Space complexity** depends on the **memory size** and the number of variables the algorithm uses.
...
Big O notation is written as `O(f(n))`, where `f(n)` represents **the growth rate** of the algorithm and `n` represents the **input size**. The `O` **symbolizes the upper bound** of the growth rate.
